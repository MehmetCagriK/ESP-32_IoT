...
#define CHAR_USER_DESC_UUID     "00002901-0000-1000-8000-00805F9B34FB"
...
void setup() {
  ...
  // Sometimes, characteristics need human readable information about their purpose.
  // User description descriptor can be used to convey this information.
  // Below, we are creating user description for the Model Number and binding it to the characteristic
  BLEDescriptor *pDoorStatusDesc = new BLEDescriptor(CHAR_USER_DESC_UUID);
  pDoorStatusDesc->setValue("Door Status");
  pDoorStatusChar->addDescriptor(pDoorStatusDesc);
  ...
  // Below, we are creating user description for the Manufacturer Name and binding it to the characteristic
  BLEDescriptor *pManNameDesc = new BLEDescriptor(CHAR_USER_DESC_UUID);
  pManNameDesc->setValue("Manufacturer Name");
  pManNameChar->addDescriptor(pManNameDesc);
  ...
  // Below, we are creating user description for the Model Number and binding it to the characteristic
  BLEDescriptor *pModelNoDesc = new BLEDescriptor(CHAR_USER_DESC_UUID);
  pModelNoDesc->setValue("Model Number");
  pModelNoChar->addDescriptor(pModelNoDesc);
